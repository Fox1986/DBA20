<!-- Dieses Script dient der Änderung von persönlichen Daten -->
...
<?php 																						
	...
	{	
		...
		if ($passw != $passw2)														/* Sicherstellen dass beide Passwörter übereinstimmen, um User nicht auszuschließen */
		{
			...
			$speicherPfad = $speicherOrt.$person.'.png';								/* Speicherformat. Datei wird nach Nickname benannt und ist damit*/
																						/* einmalig. Endung wird immer in png umgeschrieben */
		...
		if (!$fehler)																			/* Bei Fehlerfreiheit wird der Nutzer in die Datenbank aufgenommen */
		{
			...
			move_uploaded_file($_FILES['Bild']['tmp_name'], $speicherPfad);						/* Das Profilbild an in den Ordner Avatar auf dem Server hochladen */
